...
1. **The eternal optimist: the Alpha Model.** It focuses only on return and is responsible for predicting which assets are likely to rise or fall.
2. **The deeply pessimistic worrier: the Risk Model.** Its role is to keep objecting, constantly asking what might go wrong, limiting exposures, and preventing the whole system from capsizing in a bad scenario.
3. **The meticulous penny-pincher: the Transaction Cost Model.** Every trade comes with friction, slippage, impact, fees. This module behaves like a careful accountant, making sure that apparent profits are not quietly eaten away by execution costs.
4. **The rational arbitrator: the Portfolio Construction Model.** It listens to the first three, weighs return opportunities against risk and trading cost, and decides what the final portfolio should actually look like.
5. Finally comes the **Execution Model**, which acts like a highly efficient operator, carrying the orders into the market at the lowest feasible cost.
